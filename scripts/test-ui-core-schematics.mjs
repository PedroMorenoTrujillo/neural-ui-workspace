...
const collectionPath = join(workspaceRoot, 'dist', 'neural-ui', 'core', 'schematics', 'collection.json');
...
assert.equal((config.match(/provideNeuralUI\(/g) || []).length, 1, 'ng add must register one provider');
assert.equal((angularJson.match(/@neural-ui\/core\/styles/g) || []).length, 1, 'ng add must register one stylesheet');
assert.match(angularJson, /src\/styles\.scss/, 'ng add must preserve consumer styles');

const themed = await runner.runSchematic('theme', { density: 'compact', theme: 'high-contrast' }, twice);
...
for (const schematic of ['layout', 'dashboard', 'crud-page']) {
  generated = await runner.runSchematic(schematic, { name: `account-${schematic}`, project: 'app' }, generated);
}
