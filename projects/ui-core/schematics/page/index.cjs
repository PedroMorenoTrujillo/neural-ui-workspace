...
const dasherize = (value) => String(value).trim().replace(/([a-z0-9])([A-Z])/g, '$1-$2').replace(/[^a-zA-Z0-9]+/g, '-').replace(/^-|-$/g, '').toLowerCase();
const classify = (value) => dasherize(value).split('-').map((part) => part[0].toUpperCase() + part.slice(1)).join('');
...
async function destination(tree, options) {
  ...
  else entry = [...workspace.projects].find(([, project]) => project.extensions.projectType === 'application');
  if (!entry?.[1]) throw new SchematicsException('No Angular application project was found. Use --project.');
...
function sources(kind, name) {
  ...
  const angularImports = kind === 'crud'
