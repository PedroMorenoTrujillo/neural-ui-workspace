...
describe('NeuStepperComponent', () => {
  ...
  it('should expose label and description in aria-label', () => {
    const steps: NeuStepperStep[] = [{ label: 'Paso 1', description: 'Rellena tus datos' }];
    const { f } = mk({ steps });
    const button: HTMLButtonElement = f.nativeElement.querySelector('.neu-stepper__step-btn');
    expect(button.getAttribute('aria-label')).toBe('Paso 1 - Rellena tus datos');
  });
  ...
  it('next() advances step and emits stepChange', () => {
    const { comp } = mk({ activeStep: 0 });
    const emitted: number[] = [];
    comp.stepChange.subscribe((s: number) => emitted.push(s));
    comp.next();
    expect(emitted).toContain(1);
  });

  it('prev() goes back one step and emits stepChange', () => {
    const { comp } = mk({ activeStep: 1 });
    const emitted: number[] = [];
    comp.stepChange.subscribe((s: number) => emitted.push(s));
    comp.prev();
  ...
  it('prev() on first step should not go below 0', () => {
    const { comp } = mk({ activeStep: 0 });
    const emitted: number[] = [];
    comp.stepChange.subscribe((s: number) => emitted.push(s));
    comp.prev();
    // stepChange should NOT have been emitted (already at first step)
    expect(emitted.length).toBe(0);
  });

  it('goTo() navigates to a specific step', () => {
    const { comp } = mk({ activeStep: 0 });
    const emitted: number[] = [];
    comp.stepChange.subscribe((s: number) => emitted.push(s));
    comp.goTo(2);
  ...
  it('goTo() should not emit when the target step is disabled', () => {
    const steps: NeuStepperStep[] = [
      { label: 'Paso 1' },
      { label: 'Paso 2', disabled: true },
      { label: 'Paso 3' },
  ...
  it('should react to matchMedia changes and unregister the listener on destroy', () => {
    ...
    window.matchMedia = vi.fn().mockImplementation(() => ({
      ...
      removeEventListener: (_type: string, listener: (event: MediaQueryListEvent) => void) => {
        removeEventListener();
        expect(listener).toBe(changeHandler);
      },
      addListener: vi.fn(),
    ...
    try {
      ...
      expect(f.componentInstance.isCompact()).toBe(true);
      expect(addEventListener).toHaveBeenCalledTimes(1);

      changeHandler?.({ matches: false } as MediaQueryListEvent);
      expect(f.componentInstance.isCompact()).toBe(false);
  ...
  it('next() on last step should not exceed steps length', () => {
    const { comp } = mk({ activeStep: 2 });
    const emitted: number[] = [];
    comp.stepChange.subscribe((s: number) => emitted.push(s));
    comp.next();
    // stepChange not emitted when already on last step (index 2 = last of 3 steps)
    expect(emitted.length).toBe(0);
  });
});
