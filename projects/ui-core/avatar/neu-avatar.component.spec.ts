...
describe('NeuAvatarComponent', () => {
  ...
  it('initials computed from name string', () => {
    const df = TestBed.createComponent(NeuAvatarComponent);
    df.componentRef.setInput('name', 'Pedro Moreno');
    df.detectChanges();
    expect((df.componentInstance as any).initials()).toContain('P');
  });

  it('imgError fallback to initials when image fails', async () => {
    const df = TestBed.createComponent(NeuAvatarComponent);
    df.componentRef.setInput('src', 'https://invalid-url.example.com/img.png');
    df.componentRef.setInput('name', 'Test User');
    df.detectChanges();
    await df.whenStable();
    ...
    // After error, imgError should be true and initials span shown
    const initialsEl = df.nativeElement.querySelector('.neu-avatar__initials');
    expect(initialsEl).toBeTruthy();
  });

  it('color input adds color class when no src', async () => {
    const df = TestBed.createComponent(NeuAvatarComponent);
    df.componentRef.setInput('name', 'Ana');
    df.componentRef.setInput('color', 'blue');
    df.detectChanges();
    await df.whenStable();
    // Color class is on inner span.neu-avatar, not on the host
    expect(df.nativeElement.querySelector('.neu-avatar--blue')).toBeTruthy();
  });
});
